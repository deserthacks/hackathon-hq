...
function createApplicationConfig(role, date, callback) {
    ...
    async.each(years, function(year, cb) {
      ...
      async.parallel({
        attendee: function(callback) {
          createApplicationConfig('participant', startDate, function(err, application) {
        ...
        mentor: function(callback) {
          createApplicationConfig('mentor', startDate, function(err, application) {
            if (err) callback(err, null);
        ...
        sponsor: function(callback) {
          createApplicationConfig('sponsor', startDate, function(err, application) {
            if (err) callback(err, null);
        ...
        volunteer: function(callback) {
          createApplicationConfig('volunteer', startDate, function(err, application) {
            if (err) callback(err, null);
        ...
        var seed = {
          ...
          applications: {
            attendee: applications.attendee._id,
            mentor: applications.mentor._id,
            sponsor: applications.sponsor._id,
            volunteer: applications.volunteer._id
          },
