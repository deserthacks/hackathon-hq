...
describe('User Requests', function() {
  ...
  describe('GET /users/me', function() {
    beforeEach(function(done) {
      action = request(app).get('/users/me');
      done();
    });
  ...
  describe('PUT /users/:id', function() {
    beforeEach(function(done) {
      factory.create('user', function(doc) {
        user = doc;
        action = request(app).put('/users/' + user.id);
        done();
      });
    });
  ...
  describe('PUT /users/:id/verify', function() {
    beforeEach(function(done) {
      factory.create('user', function(doc) {
        user = doc;
        action = request(app).put('/users/' + user.id + '/verify');
        mock = nock('https://api.sendgrid.com')
          .post('/api/mail.send.json')
          .reply(200, {
            message: 'success'
    ...
    it('should verify user with correct key', function(done) {
      ...
      action
        .set('Authorization', 'Bearer ' + token)
        .query({ verifyToken: user.verificationKey })
        .send()
        .expect(204)
        .end(function(err, res) {
          if (err) return done(err);
  ...
  describe('PUT /users/:id/password', function() {
    beforeEach(function(done) {
      factory.create('user', function(doc) {
        user = doc;
        action = request(app).put('/users/' + user.id + '/password');
        done();
      });
    });
    ...
    it('updates the user\'s password', function(done) {
      ...
      action
        ...
        .end(function(err, res) {
          ...
          action = request(app)
            .post('/auth/local')
            .send({email: user.email, password: 'password'}) // TODO: fix; use actual new password
            .expect(200)
            .end(function(err, res) {
              if (err) return done(err);
  ...
  describe('DELETE /users/:id', function(done) {
    beforeEach(function(done) {
      factory.create('user', function(doc) {
        user = doc;
        action = request(app).delete('/users/' + user.id);
        done();
      });
    });
