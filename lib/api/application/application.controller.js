...
var ApplicationController = {
  ...
  getConfigs: function(req, res, next) {
    ...
    query.populate('application_template');
    ...
    query.exec(function(err, configs) {
      ...
      async.each(configs, function(config, cb) {
        if (!config.application_template) {
          populatedConfigs.push(config);
          return cb();
        }

        ApplicationTemplate.populate(config.application_template, [{
          path: 'modules'
        }], function(err, applicationTemplate) {
          if (err) next(err);
          console.log('applicationTemplate', applicationTemplate);
          config.application_template = applicationTemplate;
          populatedConfigs.push(config);
          cb();
        });
      }, function() {
  ...
  updateConfigs: function(req, res, next) {
    var query;

    // config to request object
    // Filter query
    if (req.params) {
      query = ApplicationConfig.find();

      if (req.params.configId) {
        var id = new ObjectId(req.params.configId);
