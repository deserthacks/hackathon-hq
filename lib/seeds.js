...
module.exports = function seed() {
  ...
  for (var i = 0; i < 20; i++) {
    if (i <= 9) {
      UserSeed.createUser(true, addUser);
    } else {
      UserSeed.createUser(false, addAdmin);
    }
  }
  ...
  //       Need to wait for mongoose to finish
  setTimeout(seedEverything, 5000);
  ...
  function seedEverything() {
    ...
    seedHackathons(arbitraryOrganizers, arbitraryVolunteers, function callbackHellIsReal(hackathons) {
      // Seed hackathons
      setTimeout(function butItWorks() {
...
function seedApplications(users, hackathon) {
  ...
  for (var i = 0; i < users.length; i++) {
    var application = new Application(users[i], hackathon);
